    ...
    <div class="container mx-auto px-4 md:px-0">
      <div class="grid gap-6 grid-cols-12">
        <div class="col-span-9">
          <h2 class="text-dark text-2xl font-bold">{{ servStore.services.find((i) => i.slug === $route.params.servName)?.service_name }} {{ title }} </h2>
      ...
      <div v-if="models.length" class="grid-cols-6 gap-6 grid mt-8">
        <div v-for="(item, index) in models" :key="index" class="col-span-3 md:col-span-1">
          <nuxt-link :to="`/services/${$route.params.serv}/${$route.params.brand.toString().toLocaleLowerCase().replaceAll('_', '-')}/${item.id.toLocaleLowerCase().replaceAll('_', '-')}`">
